...
public class Bunny : Monster
{
    ...
    void Update()
    {
        if (!isHit)
        {
            rb.velocity = new Vector2(-transform.localScale.x * moveSpeed, rb.velocity.y); //몬스터 기본 움직임

            if (!Physics2D.OverlapCircle(wallCheck[0].position, 0.01f, layerMask) &&  //벽체크 0번 플랫폼이있고
